...
for _, var in ipairs({'32', '64'}) do
    ...
    local methods = {
        update = function(self, str)
            ...
            return ffi.C['XXH' .. var .. '_update'](self, str, #str) == ffi.C.XXH_OK
        end,
        result = function(self)
            return ffi.C['XXH' .. var .. '_digest'](self)
        end,
        clear = function(self, seed)
            if check_unsigned(seed) == nil then
                error("Usage xxhash" .. var .. ":clear(unsigned number)")
            end
            return ffi.C['XXH' .. var .. '_reset'](self, seed) == ffi.C.XXH_OK
        end,
        copy = function(self)
            local copy = xxHash.new()
            ffi.C['XXH' .. var .. '_copyState'](copy, self)
            return copy
        end,
    }
    ...
    setmetatable(xxHash, {
        __call = function(_, str, seed)
            ...
            return ffi.C['XXH' .. var](str, #str, seed)
        end,
    })
